...
def main(constraint):
    ...
    res = []
    max_len = 0
    filename = '/scratch/wiki_dumps/expr_with_matching/' + constraint  + '/data/altered.json'
    all_conversations = {}
    inputs = []
    ...
    with open('/scratch/wiki_dumps/expr_with_matching/' + constraint  + '/data/all.json') as f:
        for i, line in enumerate(f):
            conv_id, clss, conversation = json.loads(line)
            all_conversations[conv_id] = conversation 
    ...
    for i, line in enumerate(inputs):
        ...
        matched = conversation['action_feature'][0]['bad_conversation_id']
        if order:
...
#    conversations_as_json_job1.csv
    ...
    df.to_csv('/scratch/wiki_dumps/expr_with_matching/%s/annotations/conversations_as_json_paired_altered.csv'%(constraint), chunksize=5000, encoding = 'utf-8', index=False, quoting=csv.QUOTE_ALL)
