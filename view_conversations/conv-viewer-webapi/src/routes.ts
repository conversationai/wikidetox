...
const SEARCH_BY_TYPE = new runtime_types.RuntimeStringType<string>(
  'SearchBy', /^(conversation_id|rev_id|page_id|page_title)$/);

const SEARCH_OP_TYPE = new runtime_types.RuntimeStringType<string>(
  'SearchBy', /^(=|LIKE)$/);

// TODO(ldixon): in time we can maybe make this a bit smarter and support escaped quotes.
const BQ_SQL_SAFE_STRING = new runtime_types.RuntimeStringType<string>(
  'SearchBy', /^[^"]+$/);
...
export function setup(app : express.Express, conf : config.Config, bqClient : bigquery.BigQueryClient ) {
  let table = `\`${conf.bigQueryProjectId}.${conf.bigQueryDataSetId}.${conf.bigQueryTable}\``;
  ...
  app.get('/api/search/:search_by/:search_op/:search_for', async (req, res) => {
    ...
    try {
      ...
      FROM ${table}
      WHERE ${req.params.search_by} ${req.params.search_op} "${req.params.search_for}"
      LIMIT 100`;
