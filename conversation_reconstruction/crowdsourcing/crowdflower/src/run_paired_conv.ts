...
function addConversationToDom(el : HTMLElement, conv: conversation.Conversation) {
  ...
  conversationDiv.setAttribute('class', 'conversation');
  el.appendChild(conversationDiv);
  conversation.walkDfsComments(rootComment,
    (next_comment: conversation.Comment) => {
      conversationDiv.innerHTML += conversation.htmlForComment(next_comment, conv);
    });
}
...
export function runWithJqueryLoaded(this:any, $:any) : void {
  ...
  $('.a-question-to-answer').each(function (this:HTMLDivElement) {
    let jsonDataJquery = $(this).find('.json-data');
    let convs = JSON.parse(jsonDataJquery[0].textContent);
    let conv1Jquery = $(this).find('.conversation1');
    let conv2Jquery = $(this).find('.conversation2');
    addConversationToDom(conv1Jquery[0], convs.conversation1);
    addConversationToDom(conv2Jquery[0], convs.conversation2);
  });
}
