...
class ParserContentHandler(xml.sax.ContentHandler):
  ...
  def endElement(self, name):
    ...
    if self.xml_path.element_path_eq(self.revision_reset_path):
      ...
      if not('page_id' in self.page_data and 'page_namespace' in self.page_data and 'page_title' in self.page_data):
         self.data = {}
         return
...
#        ret = {x: self.data[x] for x in entry_list} 
  ...
  def characters(self, content_text):
    ...
    if not("page_namespace" in self.page_data) or self.page_data["page_namespace"] in TALK_PAGE_NAMESPACE:
