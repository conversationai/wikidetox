...
def run(known_args, pipeline_args):
  ...
  join_query = " LEFT JOIN (SELECT ps_tmp.* FROM %s as ps_tmp INNER JOIN (SELECT MAX(rev_id) as max_rid, page_id FROM %s GROUP BY page_id) as tmp ON tmp.page_id == ps_tmp.page_id and ps_tmp.rev_id == tmp.max_rid) as page_state ON ingested.page_id == page_state.ps_tmp.page_id WHERE week==%d and year==%d ORDER BY rev_id_in_int"%(known_args.input_page_state_table,known_args.input_page_state_table,known_args.week, known_args.year)
  read_query = get_max_query + get_latest_query + join_query 
  with beam.Pipeline(options=pipeline_options) as p:

    # Read the text file[pattern] into a PCollection.
    reconstruction_results, page_states = (p | beam.io.Read(beam.io.BigQuerySource(query=get_max_query+get_latest_query+join_query, validate=True)) 
