...
def run(known_args, pipeline_args):
  ...
  jobname = 'reconstruction-long-pages-week{lw}year{ly}-week{uw}year{uy}'.format(lw=known_args.lower_week, ly=known_args.lower_year, uw=known_args.upper_week, uy=known_args.upper_year)
  ...
  with beam.Pipeline(options=pipeline_options) as p:
    ...
    if known_args.save_res_to_cloud:
       ...
       reconstruction_results | "WriteReconstructedResults" >> beam.io.WriteToText("gs://wikidetox-viz-dataflow/long_pages/reconstructed_%s"%jobname) 
...
class ReconstructConversation(beam.DoFn):
  ...
  def process(self, info):
    ...
    if not(page_state == []) and not(last_revision == []):
       #if known_args.save_input_to_cloud_storage: 
...
if __name__ == '__main__':
  ...
  parser.add_argument('--page_states_output_table',
                      ...
                      default='wikidetox-viz:wikidetox_conversations.page_states_long',
  ...
  known_args.last_revision_table = 'wikidetox_conversations.ingested_long_pages'
