...
def run(known_args, pipeline_args):
  ...
  with beam.Pipeline(options=pipeline_options) as p:
    ...
    reconstruction_results, page_states = (to_be_processed 
                  | beam.Map(lambda x: (x['page_id'], x)) | beam.GroupByKey()
                  | beam.ParDo(ReconstructConversation(), last_revision=beam.pvalue.AsSingleton(last_revision), page_state=beam.pvalue.AsSingleton(page_state)).with_outputs('page_states', main = 'reconstruction_results'))
