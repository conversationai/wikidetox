...
def run(known_args, pipeline_args):
  ...
  pipeline_args.extend([
    ...
    '--job_name=shard-long-pages',
    '--num_workers=30'])
  pipeline_options = PipelineOptions(pipeline_args)
  pipeline_options.view_as(SetupOptions).save_main_session = True

  mapping = lambda x: ((json.loads(x)['week'], json.loads(x)['year']), x)
  with beam.Pipeline(options=pipeline_options) as p:
     shards = (p | 'Read_to_be_processed' >> beam.io.ReadFromText("gs://wikidetox-viz-dataflow/ingested_long_pages/file*")
                 | beam.Partition(partition_fn, )) 
     for shard in shards:
         shard | "WriteShard" >> beam.io.WriteToText("gs://wikidetox-viz-dataflow/sharded_ingested_long_pages/%s"%jobname
...
class ReconstructConversation(beam.DoFn):
  ...
  def process(self, info):
    ...
    if last_page_state:
       ...
       if size > 10485760:
          logging.info('Row size limit exceeded: size %d'%size)
