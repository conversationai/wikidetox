...
class ParserContentHandler(xml.sax.ContentHandler):
  ...
  def __init__(self, data_reset_path, data_paths, revision_reset_path, out_file):
...
class WikipediaRevisionsIngester():
  def __init__(self, input_file, output_file):
    self.input_file = input_file
    self.output_file = output_file

  def run_ingester(self):
    ...
    with open(self.output_file, 'w') as f:
      ...
      cmd = (['7z', 'x', self.input_file, '-so']
             if self.input_file.endswith('.7z') else ['cat', self.input_file])
...
#  ingester = WikipediaRevisionsIngester(args.input, args.output)
